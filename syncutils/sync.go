...
type Hooks interface {
	// OnWaitStart is called right before a potentially blocking acquisition attempt starts.
	// For polling loops (*Ctx methods), this is called once at the beginning.
	OnWaitStart(name string, kind LockKind)
	// OnAcquire is called when a lock is acquired. 'waited' includes any blocking time.
	OnAcquire(name string, kind LockKind, waited time.Duration)
	// OnSlowAcquire is called after OnAcquire if waited >= threshold.
	OnSlowAcquire(name string, kind LockKind, waited, threshold time.Duration)
	// OnFail is called when acquisition fails (e.g., Try* immediate miss or ctx canceled).
